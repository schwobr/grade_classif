...
__all__ = ['BaseModule', 'Normalizer', 'NormalizerAN', 'PACSDiscriminator', 'GradesClassifModel', 'RNNAttention']
...
def _get_loss(loss_name, weight, reduction, device='cpu', nc=2):
    if loss_name == 'cross-entropy':
        loss = nn.CrossEntropyLoss(torch.tensor([weight] + [1.]*(nc - 1), device=device), reduction=reduction)
    elif loss_name == 'bce':
...
class BaseModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        self.sched = _get_scheduler(self.opt, self.hparams.sched, int(self.hparams.train_percent*self.hparams.epochs*len(self.train_dataloader())), self.lr)
        return self.opt
    ...
    def train_dataloader(self):
        if self.hparams.open_mode is not None and self.hparams.open_mode != 'RGB':
            def _collate_fn(*args, **kwargs):
                x, y = default_collate(*args, **kwargs)
                x = _open_functions[self.hparams.open_mode](x).detach()
                return [x, y]
        else:
            _collate_fn = default_collate
        return DataLoader(self.data.train, batch_size=self.bs, shuffle=True, drop_last=True, collate_fn=_collate_fn)
    ...
    def val_dataloader(self):
        ...
        if self.hparams.open_mode is not None and self.hparams.open_mode != 'RGB':
            ...
            _collate_fn = default_collate
        return DataLoader(self.data.valid, batch_size=self.bs, collate_fn=_collate_fn)
    ...
    def test_dataloader(self):
        ...
        if self.hparams.open_mode is not None and self.hparams.open_mode != 'RGB':
            ...
            _collate_fn = default_collate
        return DataLoader(self.data.test, batch_size=self.bs, collate_fn=_collate_fn) if self.data.test is not None else None
    ...
    def fit(self, **kwargs):
        ...
        trainer = pl.Trainer(gpus=self.hparams.gpus, checkpoint_callback=ckpt_callback, logger=logger, min_epochs=self.hparams.epochs,
                             max_epochs=self.hparams.epochs, train_percent_check=self.hparams.train_percent, **kwargs)
...
class Normalizer(BaseModule):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        data = (NormDataset.
                from_folder(hparams.data, extensions=['.png'], filterfunc=filt).
    ...
    def validation_end(self, outputs):
        ...
        loss = torch.stack([x['val_loss'] for x in outputs]).mean()
        log = {'val_loss': loss}
        mu_x = torch.cat([x['mu_x'] for x in outputs])
        sigma_x = torch.cat([x['sigma_x'] for x in outputs])
        mu_y = torch.cat([x['mu_y'] for x in outputs])
        sigma_y = torch.cat([x['sigma_y'] for x in outputs])
        mu_xy = torch.cat([x['mu_xy'] for x in outputs])
        m_ssim = ssim(mu_x, sigma_x, mu_y, sigma_y, mu_xy)
        m_pcc = pcc(mu_x, sigma_x, mu_y, sigma_y, mu_xy)
        m_cd = sigma_y/mu_y - sigma_x/mu_x
        log['ssim'] = m_ssim.mean().item()
        log['pcc'] = m_pcc.mean().item()
        log['cd'] = m_cd.mean().item()
        return {'val_loss': loss, 'log': log}
...
class NormalizerAN(BaseModule):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        if 'cbr' in hparams.discriminator:
            ...
            base_model = timm.create_model(hparams.discriminator, pretrained=not hparams.rand_weights)
            cut = -2
        ...
        self.loss = _get_loss(hparams.loss, 1, hparams.reduction, device=self.main_device, nc=3)
...
class PACSDiscriminator(BaseModule):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.filt = filt
        data = (ImageClassifDataset.
                     from_folder(hparams.data, label_func, classes=['04', '05', '08'], extensions=['.png'], include=['1', '3'], filterfunc=filt).
                     split_by_csv(hparams.data_csv))
        n = len(data.train)
        weights = [np.float32(n/(data.train.labels == '04').sum()),
                   np.float32(n/(data.train.labels == '05').sum()),
    ...
    def train_dataloader(self):
        ...
        if sm > 0:
            ...
            sampler = RandomSampler(self.data.train)
        return DataLoader(self.data.train, batch_size=self.bs, sampler=sampler, drop_last=True, collate_fn=_collate_fn)
    ...
    def validation_end(self, outputs):
        ...
        loss = torch.stack([x['val_loss'] for x in outputs]).mean()
        log = {'val_loss': loss}
        t0 = torch.stack([x['t0'] for x in outputs]).sum()
        f0 = torch.stack([x['f0'] for x in outputs]).sum()
        t1 = torch.stack([x['t1'] for x in outputs]).sum()
        f1 = torch.stack([x['f1'] for x in outputs]).sum()
        t2 = torch.stack([x['t2'] for x in outputs]).sum()
        f2 = torch.stack([x['f2'] for x in outputs]).sum()
        ...
        for metric in self.metrics:
            ...
            for c in range(3):
                c_name = name + f'_{c}'
                log[c_name] = metric(t[c], f[c], t[:c].sum()+t[c+1:].sum(), f[:c].sum()+f[c+1:].sum())
        return {'val_loss': loss, 'log': log}
...
class GradesClassifModel(BaseModule):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.data = (ImageClassifDataset.
                     from_folder(hparams.data, lambda x: x.parts[-3], classes=['1', '3'], extensions=['.png'], include=['1', '3'], filterfunc=filt).
                     split_by_csv(hparams.data_csv).
        ...
        self.loss = _get_loss(hparams.loss, weight if hparams.sample_mode == 0 else 1., hparams.reduction, device=self.main_device, nc=2)
